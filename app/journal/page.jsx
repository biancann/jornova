          ...
          <div className="grid grid-cols-1 md:grid-cols-3 h-full gap-6">
              ...
              <div className="divide-y bg-white rounded-lg shadow">
                {journals.map((i, idx) => (
                  <button
                    ...
                    className={`${i?.id === journal?.id ? "bg-indigo-100" : "bg-white hover:bg-gray-50"} w-full p-2.5 flex items-center gap-3 text-left ${idx === 0 && "rounded-t-lg"} ${
            ...
            {journal ? (
              <div className="md:col-span-2 bg-white rounded-lg shadow">
								...
								<div className={`p-4 md:p-6 relative ${inter.className}`}>
									<div className="absolute -top-4 bg-gray-200 text-gray-600 inline-flex px-2 py-1 rounded text-sm font-medium">
										<p>{getMood(journal.mood)}</p>
									</div>
									<div className="space-y-2">
										{journal.messages.map((i, idx) => (
											<p key={idx} className={`${i.role === "user" ? "text-gray-800" : "text-indigo-400"}`}>{i.content}</p>
										))}
									</div>
								</div>
							</div>
